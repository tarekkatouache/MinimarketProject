...
export const getUserById = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: "Internal server error while fetching user", error });
  } 
};
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: "Internal server error while deleting user", error });
  }
};